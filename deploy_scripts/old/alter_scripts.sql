...
DROP FUNCTION IF EXISTS schooldev.get_filtered_bulk_appointments;
...
DROP VIEW IF EXISTS schooldev."COMPLETE_STUDENT_APPLICATION_VIEW_BACKUP_JUN19";
DROP VIEW IF EXISTS schooldev."COMPLETE_STUDENT_APPLICATION_VIEW_BACKUP_FEB2019";
...
ALTER TABLE schooldev.roles ADD CONSTRAINT roles_pkey PRIMARY KEY (role_id);
ALTER TABLE schooldev."MENU_LIST" ADD CONSTRAINT "MENU_LIST_pkey" PRIMARY KEY (menu_id);
...
CREATE TABLE IF NOT EXISTS schooldev."PAYMENT_GATEWAY_CONFIGURATION_CCAV" (
    id bigserial NOT NULL,
...
create type hdccomplaintlisttype as (hostel_id bigint,warden_id character varying,hdc_id integer,student_id character varying,
...
create type schooldev.mess_list as (smd_id bigint, mmc_id bigint, student_id character varying, student_name character varying, mess_name character varying,
                                    ...
                                    smd_current_active_flag character varying, smd_remarks character varying);
...
ALTER TABLE schooldev."MESS_MASTER" ADD CONSTRAINT "MESS_MASTER_pkey" PRIMARY KEY (mess_master_id);
...
ALTER TABLE schooldev."ASSET_INVENTORY_INFO" ALTER COLUMN asset_id TYPE bigint USING asset_id::bigint;
...
ALTER TABLE schooldev."FACULTY_PERSONAL_DETAILS" ADD CONSTRAINT "FACULTY_PERSONAL_DETAILS_pkey" PRIMARY KEY (faculty_id);
...
ALTER TABLE schooldev."HOSTEL_ROOM_ALLOTMENT_INFO" RENAME COLUMN n_hral_new_roomallotmentid TO new_room_allotment_id;
...
ALTER TABLE schooldev."IIT_W_STUDENT_APPOINTMENT_REQUEST" ALTER COLUMN request_id SET DEFAULT nextval('"IIT_W_STUDENT_APPOINTMENT_REQUEST_request_id_seq"');
...
ALTER TABLE schooldev."IIT_WD_DASHBOARD_TAB_MASTER" ALTER COLUMN id SET DEFAULT nextval('schooldev.roles_role_id_seq');
...
AS SELECT ( SELECT dost_election_department.dept_name
          ...
          CASE
              WHEN "STUDENT_BIO_DATA_FORM_DETAILS".student_name IS NOT NULL
                  THEN "STUDENT_BIO_DATA_FORM_DETAILS".student_name::varchar(120)
              ELSE
                  ("STUDENT_DETAILS_INFO".first_name || ' ' || "STUDENT_DETAILS_INFO".last_name)::varchar(120)
              END AS student_name,
          CASE
          ...
          "HOSTEL_ROOM_ALLOTMENT_INFO".roomallotment_id AS room_allotment_id,
...
as
SELECT "VACATION_HOSTEL_ROOM_ALLOTMENT_INFO".room_allotment_id,
...
FROM schooldev."IIT_W_CANDIDATE_APPOINTMENT_REQUEST" car
         ...
         JOIN schooldev."VACATION_HOSTEL_ROOM_ALLOTMENT_INFO" vhra
              ON vhra.request_id::bigint = car.request_id AND vhra.stay_id::bigint = csr.stay_id AND
                 csr.stay_from = vhra.stay_from_date AND csr.stay_to = vhra.stay_to_date
UNION
SELECT vhrai.room_allotment_id,
       ...
       sdi.student_name                                           AS student_name,
       sdi.dob::date                                              AS dob,
       ...
       NULL::character varying                                    AS vacation_checkout_status,
       NULL::character varying                                    AS checked_in_date,
       sdi.pwd_status::boolean                                    AS pwd_status
...
UNION
SELECT sar.roomallotment_id::bigint                               AS room_allotment_id,
       sar.building_id,
       sar.room_id,
...
create or replace view schooldev."ROOM_OCCUPANCY_STATUS_VIEW_ALL_TYPE_ROOMS"
            (hostel_id, hostel_name, room_no, total, occupied, vacancy, official_guest_status) as
SELECT fm.hostel_id,
       fm.hostel_name,
       "HOSTEL_ROOM_INFO".room_no,
...
FROM schooldev."HOSTEL_ROOM_INFO"
         JOIN schooldev."HOSTEL_FLOOR_MASTER" ON "HOSTEL_FLOOR_MASTER".floor_id = "HOSTEL_ROOM_INFO".building_id AND
         ...
         LEFT JOIN schooldev."COMPLETE_HOSTEL_ALLOTMENT_VIEW"
                   ON "COMPLETE_HOSTEL_ALLOTMENT_VIEW".room_id = "HOSTEL_ROOM_INFO".room_id AND
                      "COMPLETE_HOSTEL_ALLOTMENT_VIEW".stay_from_date <= now()::date AND
                      (COALESCE("COMPLETE_HOSTEL_ALLOTMENT_VIEW".stay_to_date,
                                "COMPLETE_HOSTEL_ALLOTMENT_VIEW".shifted_date) >= now()::date OR
                       "COMPLETE_HOSTEL_ALLOTMENT_VIEW".stay_to_date IS NULL AND
                       "COMPLETE_HOSTEL_ALLOTMENT_VIEW".shifted_date IS NULL) AND
                      "COMPLETE_HOSTEL_ALLOTMENT_VIEW".vacation_category::text = 'N'::text AND
                      "COMPLETE_HOSTEL_ALLOTMENT_VIEW".active_flag = 'Y'::bpchar
...
create or replace view schooldev."DYNAMIC_USER_TABS_DISPLAYABLE_VIEW"
            (id, role, show_hide, l1_id, l1_type, l1_name, l1_property, l1_order, l1_url, l2_id, l2_type, l2_name,
             l2_property, l2_order, icon, style, url, sort, mandatory, action, user_id)
as
...
FROM schooldev."IIT_WD_DASHBOARD_TAB_MASTER" tm
         ...
         LEFT JOIN schooldev."IIT_WD_ROLE_TAB_SETTINGS" rts
                   ON (tm.id = rts.tab_id OR tm2.id = rts.tab_id) AND rts.active_flag = 'Y'::bpchar
         LEFT JOIN schooldev."IIT_WD_USER_TAB_SETTINGS" uts
                   ON (tm.id = uts.tab_id OR tm2.id = uts.tab_id) AND uts.active_flag = 'Y'::bpchar
         LEFT JOIN schooldev."USER_MANAGEMENT" um ON um.user_id::text = uts.user_id::text
WHERE (tm.parent_id = 0 OR tm.parent_id IS NULL)
  AND tm.active_flag = 'Y'::bpchar
GROUP BY rts.role, (COALESCE(uts.show_hide, rts.show_hide, false)), tm.id, tm.tab_type, tm.tab_name, tm.property,
         tm.order_by, tm.tab_url, tm2.id, tm2.tab_type, tm2.tab_name, tm2.property, tm2.order_by, tm.icon, tm.style,
         tm.url, tm.sort, tm.mandatory, tm.action, um.user_name, rts.show_hide, tm2.icon,
         tm2.style,
         tm2.url,
         tm2.sort,
         tm2.mandatory,
...
$$
DECLARE
    res mess_to_card_request_list%rowtype;
    duration timestamp;
    res_count numeric;
BEGIN
    res_count = 0;
...
BEGIN
    ...
    then
        ...
        select get_student_current_balance into total_balance from schooldev.get_student_current_balance(NEW.student_id, 'MS');
        select get_student_current_balance into total_balance_cc from schooldev.get_student_current_balance(NEW.student_id, 'CC');
        existing = null;
        ...
        then
            ...
            UPDATE schooldev.student_net_balance SET net_balance_mess = total_balance, net_balance_card = total_balance_cc WHERE student_id = NEW.student_id;
        end if;
    end if;
...
CREATE TRIGGER mess_ledger_balance_trigger
...
CREATE OR REPLACE FUNCTION schooldev.get_filtered_bulk_appointments(p_approval_status character varying DEFAULT NULL::character varying,
                                                                    p_event_name character varying DEFAULT NULL::character varying,
                                                                    p_to_date date DEFAULT NULL::date, p_from_date date DEFAULT NULL::date, p_created_at date DEFAULT NULL::date,
    ...
    FOR r IN
        WITH distinct_appointments AS (
            ...
            FROM
                ...
                ON
                    a.bulk_appointment_id = d.bulk_appointment_id
            WHERE
                (p_approval_status IS NULL OR p_approval_status = '' OR a.approval_status = p_approval_status)
...
CREATE OR REPLACE FUNCTION schooldev.search_students_hostel(sstatus character varying, scategory character varying, app_from character varying, app_to character varying, sty_from character varying, sty_to character varying, sname character varying, sid character varying, svname character varying, svemail character varying, logintype character varying, submitted_from character varying, submitted_to character varying, approval_from character varying, approval_to character varying, facilityid integer, loginid character varying, tab integer, current_stay_flag character varying)
    RETURNS SETOF student_record_result_hostel
    LANGUAGE plpgsql
AS $function$
declare
    r student_record_result_hostel%rowtype;
    updated_login_id character varying :=lower(logintype);
    facility_ids character varying;
BEGIN
    if(logintype='Warden') then
...
--raise notice 'Login:%',logintype;
    ...
    if (app_from is null or app_from='null') and (app_to is null or app_to='null') and
       ...
       (sid is null or sid='') and (sname is null or sname='') and
       (svemail is null or svemail='') and (facilityid=0) then
        if(tab=2) then scategory='outsidecampus,insidecampus,sasthra,other'; END IF;
        if (tab=10) then scategory='SCHOLAR'; END IF;
        if (tab=18) then scategory='stustayextension'; END IF;
        raise notice 'if:';
--if(lower(logintype) like '% office') then raise notice '1:';
...
---------------------------------------------------------
        ...
        if(tab=2) then if(scategory is null or scategory='') then scategory='outsidecampus,insidecampus,sasthra,other'; end if; END IF;
        if (tab=10) then if(scategory is null or scategory='') then scategory='SCHOLAR'; end if; END IF;
        if (tab=18) then if(scategory is null or scategory='') then scategory='stustayextension'; end if; END IF;
        raise notice 'else:';
        for r in
    ...
    if(userrole='CCW DEAN' OR userrole='DOST DEAN') then
        ...
        then
            ...
            for res in
                ...
                Select c.id,a.ldap_username,c.created_at::Date, c.warden_name, d.mess_name, cleanliness_kitchen, cleanliness_plate, queue_maintainance, hygiene_mess, availability_food, feedback_student, other_item FROM schooldev."WARDEN_INFO" a join schooldev."mess_inspection_report" c on (a.id = c.warden_id) left join schooldev."MESS_MASTER" d on (d.mess_master_id=c.mess_master_id) where a.active_flag = 'Y' and c.active_status = 'Y'
            ...
            for res in
                ...
                where a.active_flag = 'Y' and c.active_status = 'Y'
        ...
        then
            ...
            for res in
                ...
                FROM schooldev."WARDEN_INFO" a
                         join schooldev."mess_inspection_report" c on (a.id = c.warden_id)
                         left join schooldev."MESS_MASTER" d on (d.mess_master_id=c.mess_master_id)
                where a.active_flag = 'Y' and c.active_status = 'Y' and
            ...
            for res in
                ...
                FROM schooldev."WARDEN_INFO" a
                         join schooldev."mess_inspection_report" c on (a.id = c.warden_id)
                         left join schooldev."MESS_MASTER" d on (d.mess_master_id=c.mess_master_id)
                where a.active_flag = 'Y' and c.active_status = 'Y'  and a.id=wardenid
...
create or replace function schooldev.search_students_rebate(sstatus character varying, approval_from character varying, approval_to character varying, submitted_from character varying, submitted_to character varying, reb_from character varying, reb_to character varying, sname character varying, sid character varying, svname character varying, svemail character varying, loginid character varying, slno_from character varying, slno_to character varying, userrole character varying, userlogin character varying) returns SETOF search_students_rebate_result
    language plpgsql
as
$$
declare
...
BEGIN
    if(userrole!='SENIOR COOK') then
        if(loginid='Dean' or loginid='SoftwareAdmin') then
/*For mess.ohm login*/
...
SELECT setval('IIT_W_CANDIDATE_FILES_INFORMATION_file_id_seq', (SELECT MAX(file_id) FROM schooldev."IIT_W_CANDIDATE_FILES_INFORMATION"));
