...
import com.iitm.hosteldine.service.reports.StudentWiseLogService;
...
@Controller
@RequestMapping(value = "${url.student.wise.log.report}")
@RequiredArgsConstructor
public class StudentWiseLogReportController {
	
	private final CommonResponseUtil commonResponseUtil;
	private final MessageSource messageSource;
	private final StudentWiseLogService studentWiseLogService;
	...
	public String getMessInspectionList(@RequestParam Map<String, String> allParams, PaginationForm form, ModelMap map, HttpServletRequest request) throws Exception {
		if (!form.isSearchFilter()) {
			List<String> filterList = List.of("studentId", "fromDate", "toDate");
		...
		return HTMLPage.STUDENT_WISE_LOG_REPORT;
	}
	...
	public void downloadRequestReport(@RequestParam Map<String, String> allParams, PaginationForm form, ModelMap map,
		...
		commonResponseUtil.getAdditionalParams(allParams, form);
		Page<StudentWiseLogReportDto> studentWiseLogReportDto = studentWiseLogService.getStudentWiseLogDetailsList(form);
		try {
			Workbook workbook = studentWiseLogService.getStudentWiseLogReport(studentWiseLogReportDto.getContent());
			ByteArrayOutputStream bos = new ByteArrayOutputStream();
			workbook.write(bos);
			workbook.close();
			byte[] excelBytes = bos.toByteArray();
			response.setContentType(FileUploadConstants.XLSX);
			String fileName = messageSource.getMessage("message.student.wise.report.file.name", null, Locale.getDefault()) + "" + FileUploadConstants.XLSX_EXTENSION;
