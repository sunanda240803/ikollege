...
import com.iitm.hosteldine.service.mess.NotDineSummaryService;
...
@Controller
@RequestMapping(value = "${url.not.dine.student}")
@RequiredArgsConstructor
public class NotDineStudentsReportController {

    private final NotDineSummaryService notDineSummaryService;
    ...
    @Value("${url.not.dine.student}")
    private String baseUrl;

    @GetMapping
    public String getNotDineStudent( PaginationForm form, ModelMap model ,HttpServletRequest request) {
        ...
        return HTMLPage.NOT_DINE_STUDENTS;
    }
    ...
    public void downloadRequestReport(@RequestBody MessDineSummaryForm searchForm, @RequestParam Map<String, String> allParams,@ModelAttribute PaginationForm pageForm, ModelMap map, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            Workbook workbook =  notDineSummaryService.getNotDineSummaryData(pageForm, searchForm);
            ByteArrayOutputStream bos = new ByteArrayOutputStream();
            workbook.write(bos);
            workbook.close();
            byte[] excelBytes = bos.toByteArray();
            response.setContentType(FileUploadConstants.XLSX);
            String fileName = messageSource.getMessage("message.not.dine.students.report.filename", null, Locale.getDefault()) +"" + FileUploadConstants.XLSX_EXTENSION;
