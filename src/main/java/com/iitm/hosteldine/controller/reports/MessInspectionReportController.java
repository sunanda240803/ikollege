...
@Controller
@RequestMapping(value = "${url.mess.inspection.report}")
@RequiredArgsConstructor
public class MessInspectionReportController {
	...
	public String getMessInspectionList(@RequestParam Map<String, String> allParams, PaginationForm form, ModelMap map, HttpServletRequest request) throws Exception {
		if (!form.isSearchFilter()) {
			List<String> filterList = List.of("wardenId", "messId", "inspectionFromDate", "inspectionToDate");
			filterList.forEach(filter -> form.getAdditionalParam().put(filter, ""));
		}
		map.addAttribute("filters", form.getAdditionalParam());
		map.addAttribute("messMap",messInspectionService.fetchMessListAsMap());
		map.addAttribute("wardenMap",messInspectionService.fetchWardenListAsMap());
		...
		return HTMLPage.MESS_INSPECTION_REPORT;
	}
	...
	public void downloadRequestReport(@RequestParam Map<String, String> allParams, PaginationForm form, ModelMap map,
		...
		commonResponseUtil.getAdditionalParams(allParams, form);
		Page<MessInspectionReportDto> messInspectionDto = messInspectionService.getMessInspectionList(form);
		try {
			Workbook workbook = messInspectionService.getMessInspectionReport(messInspectionDto.getContent());
