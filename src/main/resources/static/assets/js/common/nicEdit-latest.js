...
var nicEditorConfig = bkClass.extend({
	...
	buttonList1 : ['bold', 'italic', 'underline' ,  'left', 'center', 
	              'right', 'justify', 'ul', 'ol' ],
	iconList : {
		"xhtml" : 1,
		"bgcolor" : 2,
...
var nicUploadButton = nicEditorAdvancedButton
		.extend({
			nicURI : "photoUpload/uploadNicImage",
			...
			addPane : function() {
				...
				new bkElement("div").setStyle({
					...
					fontWeight : "bold",
					paddingBottom : "5px"
				}).setContent("Select Image/File").appendTo(A);
			...
			uploadFile : function() {
				...
				if (+B.size > 2097152) {
					alert("File Size Exceeded. Please upload below 2mb file");
					return false;
				}
//
//				if (!B || !B.type.match(/image.*/)) {
//					//alert('si'+B.file[0].size);
...
//				}
			...
			onUploaded : function(B) {
				...
				if(type=='application/pdf'){
					...
					alert("Acceptes only pdf, doc, docx, xls, xlsx and image files");
				}
...
//----------------------------------------------------
var applicationNicEditor = bkClass.extend({
	construct : function(C) {
		this.options = new nicEditorConfig();
		bkExtend(this.options, C);
		this.nicInstances = new Array();
	...
	setPanel : function(A) {
		this.nicPanel = new applicationNicEditorPanel($BK(A), this.options, this);
		this.fireEvent("panel", this.nicPanel);
		return this
	},
	nicCommand : function(B, A) {
	...
	getIcon : function(D, A) {
		var C = this.options.iconList1[D];
		var B = (A.iconFiles) ? A.iconFiles[D] : "";
		return {
			backgroundImage : "url('" + ((C) ? this.options.iconsPath : B)
					+ "')",
...
applicationNicEditor = applicationNicEditor.extend(bkEvent);
var applicationNicEditorPanel = bkClass.extend({
	construct : function(E, B, A) {
		this.elm = E;
		this.options = B;
		this.ne = A;
		this.panelButtons = new Array();
		this.buttonList1 = bkExtend([], this.ne.options.buttonList1);
		this.panelContain = new bkElement("DIV").setStyle({
			overflow : "hidden",
			width : "100%",
			border : "1px solid #cccccc",
		...
		var D = C.buttons;
		for (button1 in D) {
			this.addButton(button1, C, true)
		}
		this.reorder();
		E.noSelect()
	},
	addButton : function(buttonName, options, noOrder) {
		var button1 = options.buttons[buttonName];
		var type = (button1.type) ? eval("(typeof(" + button1.type
				+ ') == "undefined") ? null : ' + button1.type + ";")
				: nicEditorButton;
		var hasButton = bkLib.inArray(this.buttonList1, buttonName);
		if (type && (hasButton || this.ne.options.fullPanel)) {
			this.panelButtons.push(new type(this.panelElm, buttonName, options,
					this.ne));
			if (!hasButton) {
				this.buttonList1.push(buttonName)
			}
		}
	},
	findButton : function(B) {
